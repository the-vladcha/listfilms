...
class ParserIvi:
    ...
    def parser_block(self, link, title):
        ...
        if Category.objects.filter(title=category).exists() is False:
            Category(title=category, slug=slugify(category)).save()

        if Film.objects.filter(title=title, year=year).exists() is False:
